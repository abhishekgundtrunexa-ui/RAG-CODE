const { OcppEvents, OcppConstants } = require("@shared-libs/constants");
const { DateTime } = require("luxon");
const { deepClone } = require("@shared-libs/helpers");
...
const handleStartTransaction = (client) => {
  client.handle(OcppEvents.StartTransaction, async ({ params }) => {
    ...
    if (logConsoles) {
      console.log(
        `Server got ${eventName} from ${clientId}:\nconnector id: ${connectorId}\ntime: ${timestamp}\nmeterStart: ${meterStart}`,
        params
      );
    }

    let responseData = { idTagInfo: { status: "Invalid" } };
