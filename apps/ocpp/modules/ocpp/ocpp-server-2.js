...
const { handleOcppEvents } = require("./ocpp-handlers-2");
const { handleExternalOcppEvents } = require("./ocpp-external-handlers-2");
...
const saveConnectedLogs = true;

const InitOcppServer2 = (app) => {
  ...
  rpcServer.on("client", async (client) => {
    ...
    client.handle(({ method, params = {} }) => {
      ...
      if (isCgx) {
        ...
        return handleOcppEvents(method, client.identity, params);
      } else {
        // Event handling for different OCPP events
        return handleExternalOcppEvents(method, client.identity, params);
      }
    });
